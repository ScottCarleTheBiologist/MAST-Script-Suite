...
Sheet3<-read.csv("Martinez and Jernigan Modified Sheet_3.csv",header=TRUE)

Sheet4<-read.csv("MAST Naraghi Sheet4.csv",header=TRUE)
...
for(i in c(1:length(CHRLIST)))
{
  ...
  if(exists("Sheet1")){if(length(sub.1$bp.pos)!=0){plot(sub.1$cM.pos~sub.1$bp.pos,col="grey",xlim=c(0,(1.05*max(c(if(exists("Sheet1")){sub.1$bp.pos},if(exists("Sheet2")){sub.2$bp.pos},if(exists("Sheet3")){sub.3$bp.pos})))),ylim=c(0,(1.05*max(c(if(exists("Sheet1")){sub.1$cM.pos},if(exists("Sheet2")){sub.2$cM.pos})))),main=CHRLIST[i],ylab="cM",xlab="Basepairs",cex=Cexing,cex.axis=Cexing,cex.lab=Cexing,cex.main=Cexing)}}
  
  if(exists("Sheet4")){
    if(length(sub.4$Marker1)!=0){rect(sub.4$bp.pos1,-50,sub.4$bp.pos2,500,angle = 45,col = rgb(sub.4$Red,sub.4$Green,sub.4$Blue,.3),border=NA)}
  }
  
  if(exists("Sheet1")&exists("Sheet2")){if((length(sub.1$bp.pos)!=0)&(length(sub.2$bp.pos)!=0)){par(new=T)}}
  if(exists("Sheet2")){if(length(sub.2$bp.pos)!=0){plot(sub.2$cM.pos~sub.2$bp.pos,col="red",xlim=c(0,(1.05*max(c(if(exists("Sheet1")){sub.1$bp.pos},if(exists("Sheet2")){sub.2$bp.pos},if(exists("Sheet3")){sub.3$bp.pos})))),ylim=c(0,(1.05*max(c(if(exists("Sheet1")){sub.1$cM.pos},if(exists("Sheet2")){sub.2$cM.pos})))),main=CHRLIST[i],ylab="cM",xlab="Basepairs",cex=Cexing,cex.axis=Cexing,cex.lab=Cexing,cex.main=Cexing)}}
  
  if(exists("Sheet3")){
    if(length(sub.3$bp.pos)!=0){abline(v=sub.3$bp.pos,col = rgb(sub.3$Red,sub.3$Green,sub.3$Blue,1))}
    if(length(sub.3$bp.pos)!=0){text(sub.3$bp.pos,sub.3$offset,pos=3,labels=sub.3$Markers,cex=Cexing,cex.axis=Cexing,cex.lab=Cexing,cex.main=Cexing)}
